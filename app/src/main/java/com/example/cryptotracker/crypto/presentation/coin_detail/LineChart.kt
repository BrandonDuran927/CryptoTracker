...
fun LineChart(
    ...
    val visibleDataPoints = remember(dataPoints, visibleDataPointsIndices) {
        dataPoints.slice(visibleDataPointsIndices) // list = [2,3,4,5,6] -> [3,4]
    }

    val maxYValue = remember(visibleDataPoints) {
        visibleDataPoints.maxOfOrNull {
            it.y
        } ?: 0f
    }
    val minYValue = remember(visibleDataPoints) {
        visibleDataPoints.minOfOrNull {
            it.y
        } ?: 0f
    }
    ...
    Canvas(
        ...
        val maxLabelWidth = yLabelTextLayoutResults.maxOfOrNull { it.size.width } ?: 0
            ...
            if (showHelperLines) {
                drawLine(
                    ...
                    strokeWidth = if (selectedDataPointIndex == index) {
                        style.helperLinesThicknessPx * 2.5f
        ...
        yLabelTextLayoutResults.forEachIndexed { index, result ->
            val x = maxLabelWidth + horizontalPaddingPx - result.size.width
            val y = (viewPortTopY) +
